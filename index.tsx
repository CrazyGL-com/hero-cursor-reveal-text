...
interface CrtProps extends HeroComponentProps {
	primaryText?: string;
	revealText?: string;
	defaultSize?: number;
	hoverSize?: number;
...
function CursorRevealTextHero(props: CrtProps) {
	const {
		size,
		input,
		rootRef,
		...
		defaultSize = 40,
		hoverSize = 400,
		cursorFollowSpeed = 0.5,
		transitionDuration = 0.5,
		bgColor = '#0f0f0f',
		textColor = '#c299d0',
		revealBgColor = '#5e057e',
		revealTextColor = '#0f0f0f',
		fontSize = 36,
		lineHeight = 1.3,
		maxWidth = 1000,
	...
	React.useEffect(() => {
		...
		try { loadGoogleFont(headingFontFamily, { weights: ['400', '500', '600', '700', '800', '900'] }); } catch { /* */ }
	}, [headingFontFamily]);
	...
	const primaryInnerRef = React.useRef<HTMLDivElement | null>(null);
	const cursorRef = React.useRef({ x: w / 2, y: h / 2 });
	const sizeStateRef = React.useRef<number>(Math.max(10, Number(defaultSize) || 40));
	...
	useHeroAnimationFrame(rootRef, ({ delta, elapsed }) => {
		...
		if (cursorActive) {
			...
			if (reducedMotion || idleRef.current < 0.4) { tx = w * 0.5; ty = h * 0.5; }
			else {
		...
		let overText = false;
		const root = (rootRef as React.RefObject<HTMLElement>).current;
		...
		<>
			<crazygl-stage style={{ background: bgColor }}>
				<div className="crazygl-crt-layer">
					<div
						ref={primaryInnerRef}
						className="crazygl-crt-inner"
						style={{ ...innerStyle, color: textColor }}
					>
						{primaryText}
					</div>
				</div>
				<div
					ref={lensRef}
					className="crazygl-crt-revealed-layer"
					aria-hidden="true"
					style={{ background: revealBgColor }}
				>
					<div className="crazygl-crt-layer">
						<div className="crazygl-crt-inner" style={{ ...innerStyle, color: revealTextColor }}>
							{revealText}
						</div>
					</div>
				</div>
...
export default function CursorRevealText(props: any) {
	return <CrazyGLWrapper hero={CursorRevealTextHero} metadata={metadata as any} {...props} />;
}
